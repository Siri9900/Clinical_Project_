...
attrib
	...
	AQSLFL    length=$1 label='ANALYSIS BASE LINE FLAG'
...
DATA PROJ.ADQS1; 
length aphase $20 aphasen 8 param $40 paramcd $10 AVAL 8 aQSlfl $1;
MERGE proj.QS4 (IN=A) proj.adsl_f (IN=B);
BY studyid usubjid; 
if a;
/*ANALYSIS PHASE*/  
...
/* Convert character dates to numeric for analysis: ANALYSIS DATE, Analysis day*/
  ADY=QSDY;
  if QSDTC ne "" then ADT= input(scan(QSDTC,1,"T"), yymmdd10.);
...
/*AQSLFL*/
  AQSLFL=QSblfl;   
run;
proc sort data=proj.adQS1; by studyid usubjid paramcd ADT; run;
...
proc sql;
    create table proj.adQS_windowed as
...
proc sort data=PROJ.adQS_flg ;by studyid  usubjid PARAMCD AVISITN AWTDIFF descending ADY;run;

/* QSBLFL    
 ALBLFL AVISIT AVISITN AWLO AWHI AWTARGET AWTDIFF ANL01FL*/
data proj.adQS_final1;
length ANL01FL $1;
if 0 then set proj.adQS_template; 
    set proj.adQS_flg;
...
keep STUDYID DOMAIN USUBJID SITEID QSSEQ VISIT QSBLFL
TRTSDT TRTEDT TRTA TRTAN SAFFL RFICDT RANDDT APHASE APHASEN ADY ADT AVAL BASE
CHG PCHG PARAM PARAMCD AQSLFL AVISIT AVISITN AWLO AWHI AWTARGET AWTDIFF ANL01FL
;
run;
proc sort data=proj.adqs_final1 out=proj.adqs_final; by studyid usubjid paramcd qsseq;run;
